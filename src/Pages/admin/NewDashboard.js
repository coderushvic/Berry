...
const AdminPage = () => {
  ...
  const onUserImageUploaded = (url) => {
    ...
    setUserImagePreview(""); // clear uploader preview
  ...
  const onAdImageUploaded = (url) => {
    ...
    toast.success(t("adImageUploadedSuccessfully"));
  };
  ...
  const handleAdSubmit = async (e) => {
    ...
    try {
      ...
      setEditAdId(null);
      fetchAds();
      toast.success(t("adSavedSuccessfully"));
    } catch (err) {
      console.error(err);
      ...
      <section className="admin-section card">
        ...
        <form className="form-grid" onSubmit={handleUserSubmit}>
          <div className="grid-2">
            <input name="name" placeholder={t("fullName")} value={newUser.name} onChange={handleChange} required />
            ...
            <input name="chestCircumference" placeholder={t("chestCircumference")} value={newUser.chestCircumference} onChange={handleChange} />
          ...
          <textarea name="about" placeholder={t("about")} value={newUser.about} onChange={handleChange} />

          <h3>{t("contactInfo")}</h3>
          <div className="grid-2">
            <input name="telegram" placeholder={t("telegram")} value={newUser.telegram} onChange={handleChange} />
            <input name="wechat" placeholder={t("wechat")} value={newUser.wechat} onChange={handleChange} />
            <input name="phone" placeholder={t("phone")} value={newUser.phone} onChange={handleChange} />
            <input name="email" placeholder={t("email")} value={newUser.email} onChange={handleChange} />
          </div>

          <textarea name="talents" placeholder={t("talentsCommaSeparated")} value={newUser.talents} onChange={handleChange} />

          <div className="checkbox-row">
            <label>
              <input type="checkbox" name="verified" checked={newUser.verified} onChange={handleChange} /> {t("verified")}
            </label>
            <label>
              <input type="checkbox" name="online" checked={newUser.online} onChange={handleChange} /> {t("online")}
            </label>
          </div>

          <div className="uploader-box">
            <label>{t("profileImage")}</label>
            <ImageUploader initialImage={userImagePreview} onUploaded={onUserImageUploaded} prominent buttonText={t("uploadProfile")} />
          </div>
        ...
        <div className="list-grid">
          {users.map((u) => (
            <div key={u.id} className="list-card">
              ...
              <h4>{u.name}</h4>
              <p>{u.age} {t("years")} • {u.status}</p>
              <div className="actions-row">
                <button className="edit-btn" onClick={() => handleUserEdit(u)}>{t("edit")}</button>
                <button className="delete-btn" onClick={() => handleUserDelete(u.id)}>{t("delete")}</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="admin-section card">
        ...
        <form className="form-grid" onSubmit={handleAdSubmit}>
          <div className="uploader-box">
            ...
            <ImageUploader initialImage={newAd.imageUrl} onUploaded={onAdImageUploaded} prominent buttonText={t("uploadAd")} />
          </div>
          <input type="text" name="link" placeholder={t("adLink")} value={newAd.link} onChange={(e) => handleChange(e, true)} />
          <input type="number" name="order" placeholder={t("order")} value={newAd.order} onChange={(e) => handleChange(e, true)} />
          <button className="submit-btn" disabled={loading}>
        ...
        <div className="list-grid">
          {ads.map((ad) => (
            <div key={ad.id} className="list-card">
              ...
              <p><strong>{t("order")}:</strong> {ad.order}</p>
              <a href={ad.link} target="_blank" rel="noreferrer">{ad.link}</a>
              <div className="actions-row">
                <button className="edit-btn" onClick={() => handleAdEdit(ad)}>{t("edit")}</button>
